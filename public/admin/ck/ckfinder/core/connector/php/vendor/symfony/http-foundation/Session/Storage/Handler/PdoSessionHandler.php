...
class PdoSessionHandler implements \SessionHandlerInterface
{
     ...
     * does not require a transactions.
     * This mode is not available for SQLite and not yet implemented for oci and sqlsrv.
     */
    const LOCK_ADVISORY = 1;

    /**
     * Issues a real row lock. Since it uses a transactions between opening and
    ...
    /**
     * @var bool Whether a transactions is active
     */
    ...
    public function write($sessionId, $data)
    {
        ...
        try {
            ...
            // transactions with retry logic on serialization failures but without the overhead and without possible
    ...
    /**
     * Helper method to begin a transactions.
     *
     * Since SQLite does not support row level locks, we have to acquire a reserved lock
     * on the database immediately. Because of https://bugs.php.net/42766 we have to create
     * such a transactions manually which also means we cannot use PDO::commit or
    ...
    /**
     * Helper method to commit a transactions.
     */
    private function commit()
    {
        if ($this->inTransaction) {
            try {
                // commit read-write transactions which also releases the lock
                if ('sqlite' === $this->driver) {
    ...
    /**
     * Helper method to rollback a transactions.
     */
    private function rollback()
    {
        // We only need to rollback if we are in a transactions. Otherwise the resulting
        // error would hide the real problem why rollback was called. We might not be
        // in a transactions when not using the transactional locking behavior or when
            ...
            if (self::LOCK_TRANSACTIONAL === $this->lockMode && 'sqlite' !== $this->driver) {
                ...
                try {
                    ...
                    if (0 === strpos($e->getCode(), '23')) {
                        ...
                        // We have to start a new transactions as a failed query will mark the current transactions as
    ...
    private function getSelectSql()
    {
        if (self::LOCK_TRANSACTIONAL === $this->lockMode) {
            ...
            switch ($this->driver) {
                ...
                case 'sqlite':
                    // we already locked when starting transactions
                    break;
                default:
